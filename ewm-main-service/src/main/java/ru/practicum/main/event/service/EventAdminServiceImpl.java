...
public class EventAdminServiceImpl implements EventAdminService {
    ...
    public EventFullDto updateAdminEvent(Long eventId, AdminEventRequestDto adminEventRequestDto) {
        ...
        if (eventAdminDto.getStateAction() != null) {
            if (!event.getState().equals(State.PENDING)) {
                ...
                throw new ConflictException(
                        "Сan not modify an event that is not in the status of pending, published, or canceled");
            }
        ...
        if (eventAdminDto.getLocation() != null) {
            event.setLocation(getLocation(eventAdminDto.getLocation())
                    .orElse(saveLocation(eventAdminDto.getLocation())));
        }
    ...
    @Override
    public List<EventFullDto> readAdminEvents(List<Long> users, List<State> states, List<Long> categories,
                                              LocalDateTime start, LocalDateTime end, int from, int size) {
        log.info("updateAdminEvent - invoked");

        List<Event> events = repository.findAllByParam(
                users, states, categories, start, end, createPageRequestAsc(from, size));
