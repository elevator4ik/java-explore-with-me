package ru.practicum.main.stat.impl;
...
public class StatServiceImpl implements StatService {
    ...
    public Map<Long, Long> toView(Collection<Event> events) {
        ...
        ResponseEntity<Object> response = statClient.getAllStatistic(
                start.format(FORMATTER), LocalDateTime.now().format(FORMATTER), uris, true);

        try {
            RequestStatDto[] stats = objectMapper.readValue(
                    objectMapper.writeValueAsString(response.getBody()), RequestStatDto[].class);

            for (RequestStatDto stat : stats) {
                view.put(
    ...
    public void addHits(HttpServletRequest request) {
        statClient.create(HitDto.builder()
